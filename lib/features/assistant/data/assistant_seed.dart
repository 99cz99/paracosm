...
const _assistants = <_Assistant>[
  _Assistant(
    ...
    '你是 Paracosm 的「角色制作助手」，帮用户制作角色卡。'
        ...
        '- 角色卡可带图（两种，写进 first_mes / description / alternate_greetings 里）：\n'
        '  - 本地图：写 <img="图片名">（图片名是你起的有意义名字，如 <img="立绘">、<img="表情-开心">）。\n'
        '    用户导入时用「配图」按钮传图，会按你卡里引用的顺序逐张映射；第一张同时作头像，建议第一张放立绘/头像。\n'
        '  - 网络图：写 <img src="https://图片地址">（需可直连的图床；国内部分图床如 catbox.moe 可能加载不出）。\n'
