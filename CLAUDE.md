...
- **认知态与摘要可靠性**：`_extractState/_extractRelation` 抽取窗口为最近 **`stateExtractWindow=6`** 条（不再只看最后一轮）；`stateJson` 种子扩为认知维度（time/place/scene/environment/char_outfit/char_body/nearby_items/npcs/style{person,perspective,onomatopoeia}/facts），注入时用 `slash_commands.dart` 的 `formatStateForPrompt`/`formatRelationForPrompt` 格式化成可读块（非裸 JSON）。摘要 `summaryWindow=8`/`keepRecent=4`（短会话也生成）、`_summarize` `maxTokens=2048`（推理模型思考不再耗尽）、**空摘要不推进 `summaryIndex`**（重试一次仍空则保消息下轮再试，防丢记忆）；记忆更新改用**会话 provider**（`resolveProvider(providerId: session.providerId)`）而非默认。
