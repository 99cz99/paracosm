...
- **Schema 迁移**：改表结构要同步 `AppConfig.schemaVersion`（当前 10）+ 在 `database.dart` 的 `migration.onUpgrade` 写 `from < N` 分支，再 `dart run build_runner build` 重生成 `database.g.dart`。改主键需删表重建——关系/affinity 这类可重抽取数据直接 `deleteTable`+`createTable` 即可（数据可接受丢失）。
